...
Greedy was extracted from another project that went up today, although not for the first time: [leftlibertarian.org](https://leftlibertarian.org). Now that the Google Reader API stuff has been successfully extracted, the next step is to make the code that runs leftlibertarian.org into something anybody can use to publish their Google Reader stream as a website. Hopefully, that won't be too long in coming...
